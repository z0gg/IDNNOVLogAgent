...
const CATALOG_ENTRY={
  package:'IDNNOVLogAgent',version:'1.0.5-1006',dname:'IDNNOV Log Agent',
  ...
  url:'https://github.com/z0gg/IDNNOVLogAgent/releases/download/v1.0.5-1006/IDNNOVLogAgent-1.0.5-1006-x86_64.spk',
  size:3010560,md5:'d8982637bf8260e7727fffcc9982cb5a',
  sha256:'69ec3f1e362c0ba6bf2d1d4bfdf0a882173a975f4b912d860384609457135a92'};
...
test('1b embedded versioned catalog works without a runtime binding',async()=>{
  ...
  const j=await r.json();
  assert.equal(j.packages.length,1);
  assert.equal(j.packages[0].version,'1.0.5-1006');
});
...
test('7c DSM 7.4.1 NAS (build 90080) also receives the release',async()=>{
  for(const a of ['r1000','geminilake']){
    ...
    assert.equal(j.packages[0].version,'1.0.5-1006',a);
  }
});
