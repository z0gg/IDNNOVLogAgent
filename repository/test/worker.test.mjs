...
const CATALOG_ENTRY={
  package:'IDNNOVLogAgent',version:'1.1.2-1015',dname:'IDNNOV Log Agent',
  ...
  url:'https://github.com/z0gg/IDNNOVLogAgent/releases/download/v1.1.2-1015/IDNNOVLogAgent-1.1.2-1015-x86_64.spk',
  size:3102720,md5:'d786031275679afef3eee80fc7ec89c6',
  sha256:'a919a6cddffc5a67b5300770510000cac2d2177cb231341e82beacc9784c7d91',
...
test('1b embedded versioned catalog works without a runtime binding',async()=>{
  ...
  const j=await r.json();
  assert.equal(j.packages.length,1);
  assert.equal(j.packages[0].version,'1.1.2-1015');
...
test('7c DSM 7.4.1 NAS (build 90080) also receives the release',async()=>{
  for(const a of ['r1000','geminilake']){
    ...
    assert.equal(j.packages[0].version,'1.1.2-1015',a);
  }
});
