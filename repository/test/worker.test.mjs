...
const CATALOG_ENTRY={
  package:'IDNNOVLogAgent',version:'1.1.4-1017',dname:'IDNNOV Log Agent',
  ...
  url:'https://github.com/z0gg/IDNNOVLogAgent/releases/download/v1.1.4-1017/IDNNOVLogAgent-1.1.4-1017-x86_64.spk',
...
test('1b embedded versioned catalog works without a runtime binding',async()=>{
  ...
  const j=await r.json();
  assert.equal(j.packages.length,1);
  assert.equal(j.packages[0].version,'1.1.4-1017');
...
test('7c DSM 7.4.1 NAS (build 90080) also receives the release',async()=>{
  for(const a of ['r1000','geminilake']){
    ...
    assert.equal(j.packages[0].version,'1.1.4-1017',a);
  }
});
