...
const CATALOG_ENTRY={
  package:'IDNNOVLogAgent',version:'1.0.6-1007',dname:'IDNNOV Log Agent',
  ...
  url:'https://github.com/z0gg/IDNNOVLogAgent/releases/download/v1.0.6-1007/IDNNOVLogAgent-1.0.6-1007-x86_64.spk',
  size:3010560,md5:'9c75e22f74b493b7748efd8a89ec144e',
  sha256:'3307b1d7966ae8494fd58d60594a74e6631ec73c867bb72edfe7e742b92f174e'};
...
test('1b embedded versioned catalog works without a runtime binding',async()=>{
  ...
  const j=await r.json();
  assert.equal(j.packages.length,1);
  assert.equal(j.packages[0].version,'1.0.6-1007');
});
...
test('7c DSM 7.4.1 NAS (build 90080) also receives the release',async()=>{
  for(const a of ['r1000','geminilake']){
    ...
    assert.equal(j.packages[0].version,'1.0.6-1007',a);
  }
});
