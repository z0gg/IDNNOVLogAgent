...
const CATALOG_ENTRY={
  package:'IDNNOVLogAgent',version:'1.1.3-1016',dname:'IDNNOV Log Agent',
  ...
  url:'https://github.com/z0gg/IDNNOVLogAgent/releases/download/v1.1.3-1016/IDNNOVLogAgent-1.1.3-1016-x86_64.spk',
  size:3102720,md5:'28aecdffa5ec7929475ff3fa445cf501',
  sha256:'71257875cca7827e948fdd7c4c3e0b616296fd129d6ec8d178f34340d385a2ce',
...
test('1b embedded versioned catalog works without a runtime binding',async()=>{
  ...
  const j=await r.json();
  assert.equal(j.packages.length,1);
  assert.equal(j.packages[0].version,'1.1.3-1016');
...
test('7c DSM 7.4.1 NAS (build 90080) also receives the release',async()=>{
  for(const a of ['r1000','geminilake']){
    ...
    assert.equal(j.packages[0].version,'1.1.3-1016',a);
  }
});
