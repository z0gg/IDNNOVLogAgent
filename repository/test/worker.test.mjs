...
const CATALOG_ENTRY={
  package:'IDNNOVLogAgent',version:'1.1.10-1027',dname:'IDNNOV Log Agent',
  ...
  url:'https://github.com/z0gg/IDNNOVLogAgent/releases/download/v1.1.10-1027/IDNNOVLogAgent-1.1.10-1027-x86_64.spk',
  size:3102720,md5:'49710da85b75a46ff10400c2de437f74',
  sha256:'693b83e0aaab6fb124b2560075b010deb733d3123bf09d5e1bd900c97ee5f979',
...
test('1b embedded versioned catalog works without a runtime binding',async()=>{
  ...
  const j=await r.json();
  assert.equal(j.packages.length,1);
  assert.equal(j.packages[0].version,'1.1.10-1027');
...
test('7c DSM 7.4.1 NAS (build 90080) also receives the release',async()=>{
  for(const a of ['r1000','geminilake']){
    ...
    assert.equal(j.packages[0].version,'1.1.10-1027',a);
  }
});
