...
const CATALOG_ENTRY={
  package:'IDNNOVLogAgent',version:'1.0.4-1005',dname:'IDNNOV Log Agent',
  ...
  url:'https://github.com/z0gg/IDNNOVLogAgent/releases/download/v1.0.4-1005/IDNNOVLogAgent-1.0.4-1005-x86_64.spk',
  size:3010560,md5:'2522c11737121098de7d2670df09e11d',
  sha256:'572367afb38fc2f3971c81f35b99a0ca7e737e47c7bb21dcbfef18bb03f27b79'};
...
test('1b embedded versioned catalog works without a runtime binding',async()=>{
  ...
  const j=await r.json();
  assert.equal(j.packages.length,1);
  assert.equal(j.packages[0].version,'1.0.4-1005');
});
...
test('7c DSM 7.4.1 NAS (build 90080) also receives the release',async()=>{
  for(const a of ['r1000','geminilake']){
    ...
    assert.equal(j.packages[0].version,'1.0.4-1005',a);
  }
});
