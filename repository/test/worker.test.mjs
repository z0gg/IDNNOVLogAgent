...
const CATALOG_ENTRY={
  package:'IDNNOVLogAgent',version:'1.0.7-1008',dname:'IDNNOV Log Agent',
  ...
  url:'https://github.com/z0gg/IDNNOVLogAgent/releases/download/v1.0.7-1008/IDNNOVLogAgent-1.0.7-1008-x86_64.spk',
  size:3010560,md5:'8354d776ff77e146b2a590e5953a4c9f',
  sha256:'fafe836343b7031c618623415d068f7eded912e2d499c2b2d588028bbdec15d1'};
...
test('1b embedded versioned catalog works without a runtime binding',async()=>{
  ...
  const j=await r.json();
  assert.equal(j.packages.length,1);
  assert.equal(j.packages[0].version,'1.0.7-1008');
});
...
test('7c DSM 7.4.1 NAS (build 90080) also receives the release',async()=>{
  for(const a of ['r1000','geminilake']){
    ...
    assert.equal(j.packages[0].version,'1.0.7-1008',a);
  }
});
